...
utils.arm_takeoff(constants.TAKEOFF_ALTITUDE)
...
#! 5.------ MISSION INIT ------
class State(Enum):
    AUTO_SCANNING, AUTO_NOT_SCANNING, AUTO_TO_BRAKE, BRAKE, BRAKE_TO_GUIDED, GUIDED_TO_BRAKE, BRAKE_TO_AUTO = range(7)
...
try:
    while True:
        ...
        if len(contours) != 0:
            contour = contours[0]
            frame = cv.drawContours(frame, contours, -1, constanats.CONTOUR_OUTLINE_COLOR, CONTOUR_OUTLINE_THICKNESS)
                ...
                if contour != None:
                    ...
                    if dist_from_center > constants.CENTER_THRESHOLD:
                        ...
                        if maxFound < constants.FOUND_MIN_THRESHOLD:
                            continue
                        found_targets.append(currentFound.index(maxFound))
                        state = States.BRAKE_TO_AUTO.value
                        drone.mode = VehicleMode("AUTO")
